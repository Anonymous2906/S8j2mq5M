...
class cDownloadProgressBar(threading.Thread):
    ...
    def _StartDownload(self):
        ...
        if (TotDown == iTotalSize) and (iTotalSize > 10000):
            meta['status'] = 2
            try:
                cDownload()._send_request('updateDownload', data=meta)
...
class cDownload:
    ...
    def ResetDownload(self):
        ...
        try:
            self._send_request('resetDownload', params=meta)
            self.DIALOG.VSinfo(self.ADDON.VSlang(30071), self.ADDON.VSlang(30224), 5)
            VSupdate()
        except:
            pass
    ...
    def DelFile(self):
        ...
        if (oDialog == 1):
            ...
            try:
                self._send_request('delDownload', params=meta)
                xbmcvfs.delete(path)
    ...
    def GetNextFile(self):
        row = self._send_request('getDownload')
    ...
    def GetOnefile(self):
        ...
        row = self._send_request('getDownload', params=meta)
    ...
    def getDownloadList(self):
        ...
        for data in row:
            ...
            url = UnquotePlus(data['url'])
            path = data['path']
            thumbnail = UnquotePlus(data['icon'])
        ...
        if sTitle:
            ...
            if sPath != '':
                ...
                try:
                    ...
                    self._send_request('insertDownload', data=meta)
                    return True
