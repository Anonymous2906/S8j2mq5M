...
#        if not sTmdbId:
        ...
        if 'film' in sMedia:
            ...
            if sRes:
                ...
                bValid = False
                for res in listRes:
                    res = res.upper().replace('FHD','1080P').replace('HD', '720P').replace('4K', '2160P')
                    if sRes in res:
                        bValid = True
                        break
    ...
    for res in sorted(listRes.keys(), key=trie_res):
        for sHosterUrl, lang in listRes[res]:
            ...
            if res:
                ...
                displayRes = res.replace('P', 'p').replace('1080p', '[COLOR fuchsia]FHD[/COLOR]').replace('720p', '[COLOR seagreen]HD[/COLOR]').replace('2160p', '[COLOR red]4K[/COLOR]')
            ...
            if len(listLinks) > 0:
                ...
                for link in listLinks:
                    if idxResMovie < len(listResMovie):
                        resMovie = listResMovie[idxResMovie].upper().replace('FHD','1080P').replace('HD', '720P').replace('4K', '2160P')
