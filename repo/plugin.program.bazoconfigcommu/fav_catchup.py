...
def sauvegarder_donnees_utilisateur():
    ...
    source_path = xbmcvfs.translatePath("special://home/userdata/addon_data/plugin.video.catchuptvandmore/")
    destination_path = f"/dossier_partager/profils/{pseudo}/fav_catchup/"
    ...
    with ftp:
        try:
            ...
            favourites_path = os.path.join(source_path, "favourites.json")
            with open(favourites_path, 'rb') as f:
                ftp.storbinary(f'STOR {os.path.join(destination_path, "favourites.json")}', f)

            xbmcgui.Dialog().ok("Sauvegarde terminée", f"Le fichier favourites.json a été sauvegardé pour l'utilisateur {pseudo}")
        except ftplib.all_errors as e:
